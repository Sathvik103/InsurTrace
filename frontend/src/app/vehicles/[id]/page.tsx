...
export default function VehicleDetailPage() {
  ...
  useEffect(() => {
    async function fetchTimeline() {
      ...
      // Graceful fallback to context vehicles (handles user-registered vehicles and offline demo)
      ...
      <div className="space-y-6">
          ...
          actions={
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-sky-600 hover:bg-sky-700 text-white text-xs font-semibold transition-colors shadow-2xs"
              >
              ...
              <button
                onClick={() =>
                  router.push(
                    `/decision/extract?vehicle_reg=${encodeURIComponent(
                      vehicle.registration_number
                    )}`
                  )
                }
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-xs font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors shadow-2xs"
              >
        ...
        <div className="overflow-x-auto pb-1">
          <div className="flex items-center gap-2 min-w-max">
            ...
            <Link
              href="/vehicles/new"
              className="px-3 py-2 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 text-xs font-semibold text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 flex items-center gap-1.5 hover:bg-zinc-100/50 dark:hover:bg-zinc-800/40"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Active Policy"
            value="Comprehensive"
            subtext="Standard Indian Motor Tariff"
            icon={Building}
          />
          <StatCard
        ...
        {/* Integrity Notice */}
        <div className="p-4 rounded-xl border border-sky-100 dark:border-sky-900/40 bg-sky-50/50 dark:bg-sky-950/20 text-xs text-sky-900 dark:text-sky-300 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            <Lock className="w-4 h-4 text-sky-600 dark:text-sky-400 shrink-0" />
            <span>
              <strong>Record Integrity:</strong> {integrity_notice} Records on this timeline are
                      ...
                      <FadeIn className="mt-3 p-3.5 rounded-lg bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 space-y-2 text-xs font-mono">
                        <div className="flex items-center justify-between text-[11px]">
                          ...
                          <span className="text-zinc-800 dark:text-zinc-200">
                            Hyperledger Fabric (Dual-Peer Raft)
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-zinc-500">Integrity Check:</span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-semibold">
                            {item.blockchain_verified ? 'VERIFIED (State matches ledger)' : 'Session record'}
                          </span>
                        </div>
